...
<link rel="stylesheet" type="text/css"
	href="/gs25/assets/css/sweetalert.css">
...
<body>
	...
	<div class="container">
		...
		<div id="title1_p">
			...
			<p>
				<em id="emphasis">GS25</em>편의점 회원이실 경우, 각 해당란에 모두 기입해주십시오.
			</p>
		</div>
		<div id="contents">
			<div class="both left">
		...
		$("#btn_Login")
				.on(
						...
						function() {
							...
							if ($("#birth").val() == "") {
								sweetAlert("생년월일은 필수 입력 항목입니다.",
										"Something went wrong!", "error");
								$("#birth").focus();
								return false;
							} else {
								if (!regBirth.test($("#birth").val())) {
									sweetAlert("생년월일 입력 형식이 잘못되었습니다.",
											"Something went wrong!", "error");
									$("#birth").focus();
									return false;
								}
							...
							if ($("#phone").val() == "") {
								...
								if (!regPhone.test($("#phone").val())) {
									sweetAlert("휴대폰 번호 입력된 내용이 잘못된 형식입니다.",
											"Something went wrong!", "error");
									$("#phone").focus();
									return false;
								}
		...
		$("#btn_pass")
				.on(
						...
						function() {
							...
							if ($("#email1").val() == "") {
								sweetAlert("아이디를 입력하시지 않으셨습니다.",
										"Something went wrong!", "error");
								$("#email1").focus();
								return false;
							} else { //휴대폰 유효성 검사
								if (!regEmail.test($("#email1").val())) {
									sweetAlert("입력된 아이디 형식이 유효하지 않습니다.",
											"Something went wrong!", "error");
									$("#email1").focus();
									return false;
								}
