...
import kr.ac.sungkyul.gs25.dao.EventManageDao;
...
public class EventManageService {
	...
	// 리스팅 되는 게시물 수
			...
			public Map<String, Object> getEventList(String spage, String keyword, Long store_no) {
				// 페이지 값 받기
				 int page = Integer.parseInt(spage);

				int totalCount = eventmanageDao.getTotalCount();
				...
				// 이벤트 리스트 받기
				List<EventBoardVo> list = eventmanageDao.getEventList(page, LIST_PAGESIZE, keyword, store_no);
			...
			public void eventDelete(Long no) {
				eventmanageDao.eventDelete(no);
			}

			public void eventDelete(EventBoardVo vo) {
				eventmanageDao.eventDelete(vo);
			}
